...
def error_check():
    try:
        ...
        try:
            ...
            click.secho(
                "User data may have been corrupted or not Initialised.Please Initialise",
                fg="red",
                bold=True)
            sys.exit(0)
...
@click.option('--token', is_flag=bool, default=False,
              help='Shows the user access token stored.')
@click.option('--npa', is_flag=bool, default=False,
              help='Shows the default no of pages for given input.')
@click.option('--npo', is_flag=bool, default=False,
              help='Shows the default no of top post in the output.')
...
@click.option('--token', is_flag=bool, default=False,
              help='Edits the user access token stored.')
@click.option('--npa', is_flag=bool, default=False,
              help='Edits the no of pages.')
@click.option('--npo', is_flag=bool, default=False,
              help='Edits the no of top post.')
...
def get(page, npa, npo):
    ...
    click.clear()
    if npa == -1:
        search_res = graph.get('search?q=' + search +
                               '&type=page&limit=' + str(abs(getdata['npa'])))
    else:
        search_res = graph.get('search?q=' + search +
                               '&type=page&limit=' + str(abs(npa)))
    if search_res['data']:
        for index, item in enumerate(search_res['data']):
            # The 'data' key of 'search_res' dictionary is a list of
            # dictionaries of 5 pages
            s = '	' + str(index + 1) + ' |>  ' + item['name']
            click.secho(s, fg="blue", bold=True)
        click.secho(
            "Please enter the page no: ",
            nl=False,
            fg="green",
            bold=True)
        try:
        ...
        if npo == -1:
            variable = graph.get(pid +
                                 '/posts?fields=comments.limit(5),link,full_picture,message&limit=' +
                                 str(abs(getdata['npo'])))
        else:
            variable = graph.get(
                pid + '/posts?fields=comments.limit(5),link,full_picture,message&limit=' + str(abs(npo)))
            ...
            for i in range(0, len(variable['data'])):
                ...
                try:
                    ...
                    for j in range(0, len(
                            variable['data'][i]['comments']['data'])):
                        del variable['data'][i]['comments']['data'][j]['id']
                        del variable['data'][i]['comments']['data'][j]['from']['id']
                except BaseException:
                    pass
                try:
                    variable['data'][i]['full_picture'] = '<div style="width:500px;height:500px;overflow:scroll"><img style="width:100%;height:auto" src=\"' + \
                        variable['data'][i]['full_picture'] + '\"></div>'
                except BaseException:
                    pass
                try:
                    variable['data'][i]['link'] = '<a href=\"' + variable['data'][i]['link'] + \
                        '\">' + variable['data'][i]['message'] + '</a>'
                except BaseException:
                    variable['data'][i]['link'] = '<a href=\"' + \
                        variable['data'][i]['link'] + '\">link</a>'
                try:
